...
| SCHED-029 | V1 | PROB-001 | Each organizer-listed start time shall provide capacity for at most one tournament fixture across both venues. Both venue rows remain eligible alternatives, and the selected schedule identifies which venue receives the fixture. |
| SCHED-030 | V1 | PROB-003 | Soft scheduling objectives shall prefer a coherent cadence across the confirmed window and avoid unnecessarily packing group fixtures early while leaving a disproportionate idle gap before knockouts. Weather, rest, availability, and organizer preferences may still justify deviations. |
