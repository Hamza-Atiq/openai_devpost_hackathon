...
export function ScheduleDiffRail({ validated, unchanged, moved, added, removed, metricDeltas, versions = [], onApprove, onReject, onRestore }: Props) {
  ...
  return (
    <section className="repair-review" aria-labelledby="repair-review-title">
      ...
      <div className="repair-evidence"><section><p className="eyebrow">Metric difference · draft minus official</p>{Object.keys(metricDeltas).length ? <dl>{Object.entries(metricDeltas).map(([key, delta]) => { const config = metricLabels[key] ?? { label: key.replaceAll("_", " "), positive: "up" as const }; const improved = config.positive === "up" ? delta > 0 : delta < 0; return <div key={key}><dt>{config.label}</dt><dd className={delta === 0 ? "delta-flat" : improved ? "delta-good" : "delta-tradeoff"}>{delta > 0 ? "+" : ""}{delta.toFixed(1)}</dd></div>; })}</dl> : <p>No comparable numeric metric changed.</p>}</section><section><p className="eyebrow">Recovery explanation</p><h2>{moved.length} fixture{moved.length === 1 ? "" : "s"} moved; {unchanged.length} preserved.</h2><p>The selected venue-time slot became unavailable. The deterministic repair engine minimized changed fixtures before movement distance and quality degradation.</p><ul><li><strong>Solver evidence:</strong> {moved.length + added.length + removed.length} changed placement{moved.length + added.length + removed.length === 1 ? "" : "s"}.</li><li><strong>Validator evidence:</strong> {validated ? "all hard constraints and stage chronology passed" : "validation failed"}.</li></ul></section></div>
