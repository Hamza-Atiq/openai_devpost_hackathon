...
export function ScheduleDiffRail({ validated = true, unchanged = defaults.unchanged, moved = defaults.moved, added = defaults.added, removed = defaults.removed, metricDeltas = { weather_risk: -9.4, group_rest_fairness: 1.8, venue_balance: 0, slot_balance: -2.1, preference_satisfaction: -1.4 } }: Props) {
  return (
    <section className="repair-review" aria-labelledby="repair-review-title">
      ...
      {validated ? <div className="schedule-approval"><div><strong>Review complete?</strong><span>Approval creates a new official version. Until then, the current official schedule remains unchanged.</span></div><button className="secondary-action" type="button">Reject repair</button><button className="primary-action" type="button">Approve repaired schedule</button></div> : <div className="error-banner" role="alert">This repair cannot be approved; the official baseline remains unchanged.</div>}
    </section>
  );
}
